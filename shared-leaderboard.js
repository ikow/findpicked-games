...
(function() {
  ...
  // Fetch challenge token on page load (proves user visited the game)
  async function getToken() {
    if (cachedToken) return cachedToken;
    try {
      const res = await fetch(`${API}/token/${GAME}`);
      const data = await res.json();
      cachedToken = data.token;
      // Refresh before expiry
      setTimeout(() => { cachedToken = null; }, (data.expires || 240) * 1000);
      return cachedToken;
    } catch { return null; }
  }

  // Pre-fetch token when page loads
  getToken();
  ...
  style.textContent = `
    ...
    .lb-error { color: #ff3b30; font-size: 13px; margin-top: 8px; }
  ...
  function createModal(score, isLowerBetter) {
    ...
    const errorEl = overlay.querySelector('#lb-error');
  ...
  window.Leaderboard = {
    submitScore(score, opts = {}) {
      createModal(score, opts.lowerIsBetter || false);
    },
    getPersonalBest,
